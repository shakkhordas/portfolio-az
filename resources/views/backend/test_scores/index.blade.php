<x-app-layout>
    ...
    <div class="py-12">
        ...
        <div class="flex justify-center mb-8">
            <a href="{{ route('test-scores.create') }}" class="bg-blue-500 text-white px-6 py-3 rounded-lg shadow hover:bg-blue-600 transition font-semibold">Add Test Score</a>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                @foreach ($testScores as $testScore)
                    <div class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 flex flex-col justify-between">
                        <div>
                            <h3 class="text-xl font-semibold mb-2">{{ $testScore->name }}</h3>
                        ...
                        <div class="mt-6 flex justify-between">
                            ...
                            <a href="{{ route('test-scores.edit', $testScore) }}" class="bg-green-500 text-white px-6 py-3 rounded-lg shadow hover:bg-green-600">Edit</a>
                            <form action="{{ route('test-scores.destroy', $testScore) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this test score?');" style="display:inline;">
                                @csrf
                                @method('DELETE')
